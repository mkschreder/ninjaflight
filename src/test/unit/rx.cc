...
TEST_F(RxTest, TestStartupValues){
	...
	for(int c = 0; c < RX_MAX_SUPPORTED_RC_CHANNELS; c++){
		...
		else EXPECT_EQ(config.data.rx.midrc, rx_get_channel(&rx, c));
	}
}
...
TEST_F(RxTest, TestFailsafeOnSingleChannelLoss){
	...
	// should be failsafe
	EXPECT_EQ(config.data.rx.midrc, rx_get_channel(&rx, 1));
...
TEST_F(RxTest, TestFailsafeModes){
	...
	config.data.rx_output.failsafe[AUX1].mode = RX_FAILSAFE_MODE_HOLD;
	config.data.rx_output.failsafe[AUX2].mode = RX_FAILSAFE_MODE_HOLD;
	...
	config.data.rx_output.failsafe[AUX1].mode = RX_FAILSAFE_MODE_SET;
	config.data.rx_output.failsafe[AUX2].mode = RX_FAILSAFE_MODE_SET;

	config.data.rx_output.failsafe[AUX1].step = 13;
	config.data.rx_output.failsafe[AUX2].step = 13;
	...
	config.data.rx_output.failsafe[AUX1].mode = RX_FAILSAFE_MODE_AUTO;
	config.data.rx_output.failsafe[AUX2].mode = RX_FAILSAFE_MODE_AUTO;

	rx_run(&rx, RX_CHANNEL_TIMEOUT);

	EXPECT_EQ(config.data.rx.midrc, rx_get_channel(&rx, AUX1));
	EXPECT_EQ(config.data.rx.midrc, rx_get_channel(&rx, AUX2));
}
...
TEST_F(RxTest, ChannelRemapping){
	...
	// pwm 1 = AUX3
	// etc
	rx_config_set_mapping(&config.data.rx, "4321AETR");
...
TEST_F(RxTest, OutputRange){
	...
	// just check
	EXPECT_EQ(885, config.data.rx.rx_min_usec);
	EXPECT_EQ(2115, config.data.rx.rx_max_usec);
