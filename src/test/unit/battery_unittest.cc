...
TEST(BatteryTest, BatteryADCToVoltage)
{
	...
	struct battery_config testBatteryConfig = {
	...
	struct battery_config *config = &testBatteryConfig;
	...
	for (uint8_t index = 0; index < testIterationCount; index ++) {
		...
		config->vbatscale = batteryAdcToVoltageExpectation->scale;
#ifdef DEBUG_BATTERY
...
#endif
		uint16_t pointOneVoltSteps = _battery_adc_to_voltage(&battery, batteryAdcToVoltageExpectation->adcReading);
...
typedef struct batteryAdcToBatteryStateExpectation_s
{
	uint16_t adcReading;
	uint16_t expectedVoltageInDeciVoltSteps;
	battery_state_t expectedBatteryState;
	uint8_t scale;
...
TEST(BatteryTest, CellCount)
{
	...
	for (uint8_t index = 0; index < testIterationCount; index ++) {
		...
		config->vbatscale = batteryAdcToCellCountExpectation->scale;
		currentADCReading = batteryAdcToCellCountExpectation->adcReading;
		battery_update(&battery);
		EXPECT_EQ(batteryAdcToCellCountExpectation->cellCount, battery.batteryCellCount);
	}
}
