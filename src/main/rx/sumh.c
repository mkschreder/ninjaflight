...
bool sumhInit(rxRuntimeConfig_t *rconf, rcReadRawDataPtr *callback)
{
    if (callback)
        *callback = sumhReadRawRC;

    rconf->channelCount = SUMH_MAX_CHANNEL_COUNT;
...
static uint16_t sumhReadRawRC(rxRuntimeConfig_t *rconf, uint8_t chan)
{
    UNUSED(rconf);
