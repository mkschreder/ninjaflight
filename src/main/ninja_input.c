...
void ninja_process_rc_sticks(struct ninja *self, rxConfig_t *rxConfig, throttleStatus_e throttleStatus, bool retarded_arm, bool disarm_kill_switch){
	...
	if (!isUsingSticksToArm) {
		if (rcModeIsActive(BOXARM)) {
			...
			if (self->is_armed && rx_has_signal(&self->rx) && !failsafe_is_active(&self->failsafe)  ) {
	...
	if (isUsingSticksToArm) {
		...
		if (rcSticks == THR_LO + YAW_LO + PIT_CE + ROL_CE) {
			if (self->is_armed)
				...
				beeper_start(&self->beeper, BEEPER_DISARM_REPEAT);	// sound tone while stick held
...
*/
	...
	if (i) {
		...
		beeper_multi_beeps(&self->beeper, i);
		return;
	}
...
void ninja_process_rx(struct ninja *self){
	...
	int32_t currentTime = sys_micros(self->system);
	...
	if (throttleStatus == THROTTLE_LOW) {
		if (rcModeIsActive(BOXAIRMODE) && !failsafe_is_active(&self->failsafe) && self->is_armed) {
		...
		if (isUsingSticksForArming()) {
			if (throttleStatus == THROTTLE_LOW) {
				if (armingConfig()->auto_disarm_delay != 0
					&& (int32_t)(self->disarmAt - sys_millis(self->system)) < 0
				) {
					...
					beeper_start(&self->beeper, BEEPER_ARMED);
					armedBeeperOn = true;
				}
			} else {
				// throttle is not low
				if (armingConfig()->auto_disarm_delay != 0) {
					// extend disarm time
					self->disarmAt = sys_millis(self->system) + armingConfig()->auto_disarm_delay * 1000;
				}

				if (armedBeeperOn) {
					beeper_stop(&self->beeper);
					armedBeeperOn = false;
				}
			}
		} else {
...
// TODO: switching between angle and horizon
/*
	bool canUseHorizonMode = true;
	if ((rcModeIsActive(BOXANGLE) || (feature(FEATURE_FAILSAFE) && failsafe_is_active(&self->failsafe))) && (ninja_has_sensors(self, NINJA_SENSOR_ACC))) {
...
bool isRangeActive(struct rx *self, uint8_t auxChannelIndex, channelRange_t *range)
{
	...
	uint16_t channelValue = constrain(rx_get_channel(self, auxChannelIndex + RX_NON_AUX_CHANNEL_COUNT), CHANNEL_RANGE_MIN, CHANNEL_RANGE_MAX - 1);
